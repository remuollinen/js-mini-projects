...
// https://api.github.com/users/(username)/repos  replace the username with actual GITHUBS usernam
...
const search = document.querySelector("#github-username");
...
function getRepos(e) {
	e.preventDefault();
	repos.innerHTML = ""; // empty repos list before adding new results

	const GithubAPI = new FetchWrapper("https://api.github.com/"); // fetch github user data

	GithubAPI.get(`users/${search.value}/repos`)
		.then((data) => {
			data.forEach((repo) => {
				// go through each repo retreived from api
				// console.log(repo);
				repos.insertAdjacentHTML(
					"beforeend",
					`<li>${repo.name}<a href="${repo.html_url}" target="_blank"><span>GitHub link</span></a></li>`
				); // insert repo names to repo list
			});
		})
		.catch((error) => {
			console.error(error);
