...
// displays all the spacecrafts

document.addEventListener("DOMContentLoaded", displayAll);

function displayAll() {
	data.forEach((ship) => {
		const craft = document.createElement("li");
		craft.textContent = ship.spacecraft;
...
form.addEventListener("submit", filterShipSubmit);
input.addEventListener("keyup", filterShip);

// event handlers

// filters spacecrafts dynamically based on what user types in search field
function filterShip(event) {
	event.preventDefault();
	let userInput = event.currentTarget.value.trim();
	// console.log(userInput);
	results.innerHTML = "";
	data.filter((ship) => {
		const spacecraftLowerCased = ship.spacecraft.toLowerCase();
		if (spacecraftLowerCased.includes(userInput)) {
			const filteredCraft = document.createElement("li");
			filteredCraft.textContent = ship.spacecraft;
			results.append(filteredCraft);
		}
	});
}

// filters spacecrafts when user submits the form (presses enter)
function filterShipSubmit(event) {
	event.preventDefault();
